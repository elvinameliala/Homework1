...
class Bank:
    ...
    def generate_report_per_customer(self, customer: Customer):
        if customer is None:
            ...
            SubTotal = 0
            for account_name, account in customer.accounts.items():
                GrandTotal = SubTotal + account.balance
                print(account_name, account.balance)
            print('Balance from your account(s) is {}.'.format(GrandTotal))
